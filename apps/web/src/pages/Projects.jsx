    ...
    <div className="page">
      ...
      <div className="stats-row" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
        <div className="stat-card">
          ...
          <div className="stat-value accent">{online}</div>
        </div>
        <div className="stat-card">
        ...
        <div className="stat-card">
          <div className="stat-label">With Uncommitted</div>
          ...
          <div key={p.id} className="org-card" style={{ textAlign: 'left', padding: 18 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: 10 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span className={`org-status-dot ${p.status === 'online' ? 'active' : 'idle'}`} />
                <span className={`badge ${p.status === 'online' ? 'active' : 'pending'}`}>{p.status}</span>
              </div>
            </div>
            ...
            <div style={{ borderTop: '1px solid var(--border)', paddingTop: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              {p.github && (
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12 }}>
                  ...
                  <span className="mono" style={{ color: 'var(--blue)' }}>{p.github.split('/')[1]}</span>
                </div>
              )}
            </div>
